...
def ddf_surveys(detailers=None, season_unobs_frac=0.2, euclid_detailers=None, flush_length=2):
    obs_array = generate_ddf_scheduled_obs(season_unobs_frac=season_unobs_frac, flush_length=flush_length)
    ...
    survey1 = ScriptedSurveyFilter([], detailers=detailers)
    survey1.set_script(obs_array[all_other])

    survey2 = ScriptedSurveyFilter([], detailers=euclid_detailers)
...
def main(args):
    ...
    if dbroot is None:
        ...
        fileroot = dbroot + "_"
    file_end = "fl%02d_v3.2_" % ddf_flush_length
